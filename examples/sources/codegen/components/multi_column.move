module examples::multi_column_comp {
	...
	public(friend) fun set(_obelisk_world: &mut World, _obelisk_entity_key: address, state: vector<u8>, last_update_time: u64) {
		...
		if(table::contains<address, MultiColumnData>(&_obelisk_component.data, _obelisk_entity_key)) {
			*table::borrow_mut<address, MultiColumnData>(&mut _obelisk_component.data, _obelisk_entity_key) = _obelisk_data;
		} else {
	...
	public(friend) fun set_state(_obelisk_world: &mut World, _obelisk_entity_key: address, state: vector<u8>) {
		let _obelisk_component = world::get_mut_comp<CompMetadata>(_obelisk_world, id());
		assert!(table::contains<address, MultiColumnData>(&_obelisk_component.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow_mut<address, MultiColumnData>(&mut _obelisk_component.data, _obelisk_entity_key);
	...
	public fun get(_obelisk_world: &World ,_obelisk_entity_key: address): (vector<u8>,u64) {
  		...
  		assert!(table::contains<address, MultiColumnData>(&_obelisk_component.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow<address, MultiColumnData>(&_obelisk_component.data, _obelisk_entity_key);
		(
			_obelisk_data.state,
	...
	public fun contains(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_component = world::get_comp<CompMetadata>(_obelisk_world, id());
		table::contains<address, MultiColumnData>(&_obelisk_component.data, _obelisk_entity_key)
	}
